...
local function render_cell(nb, cell, range, width, win)
  ...
  -- Left border: sign_text (works on wrapped continuation rows when supported).
  -- Right border: virt_text_pos="right_align" — places at the rightmost
  -- displayed column regardless of width math (more reliable than win_col
  -- when actual gutter width differs from our calculation).
